...
class TagProbeFitter {
  ...
  TagProbeFitter(const std::vector<std::string>& inputFileNames, std::string inputTreeName, std::string outputFileName, int numCPU = 1, bool saveWorkspace_ = false, bool floatShapeParameters = true, const std::vector<std::string>& fixVars_ = std::vector<std::string>() );
  ...
  void saveFitPlot(RooWorkspace* w, bool do_log);
  ...
  ///makes the 1D hist
  void makeEfficiencyHist1D(RooDataSet& eff, RooRealVar& v, const TString& plotName, const TString& plotTitle, const TString& effName, const char *catName = nullptr, int catIndex = -1);
