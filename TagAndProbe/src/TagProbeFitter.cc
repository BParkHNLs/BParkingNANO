...
TagProbeFitter::TagProbeFitter(const std::vector<std::string>& inputFileNames, string inputTreeName, string outputFileName, int numCPU_, bool saveWorkspace_, bool floatShapeParameters_, const std::vector<std::string>& fixVars_){
  ...
  std::cout << "In TagProbeFitter::TagProbeFitter ; tree size: " << inputTree->GetEntries() << std::endl;
  outputFile = new TFile(outputFileName.c_str(),"recreate");
  std::string outputDirectoryName = "results";
  outputDirectory = outputFile->mkdir(outputDirectoryName.c_str());
  numCPU = numCPU_;
...
bool TagProbeFitter::addVariable(string name, string title, double low, double hi, string units){
  std::cout << "In TagProbeFitter::addVariable ; variable name: " << name << std::endl;
...
void TagProbeFitter::saveEfficiencyPlots(RooDataSet& eff, const TString& effName, RooArgSet& binnedVariables,RooArgSet& mappedCategories){
  ...
  for(RooRealVar* v1 = (RooRealVar*)v1it.Next(); v1!=nullptr; v1 = (RooRealVar*)v1it.Next() ){
    ...
    if(allCats1D.numTypes()==0){
      ...
      makeEfficiencyHist1D(eff, *v1, TString::Format("%s_HIST", v1->GetName()), "", effName);
    }else{
      RooDataSet myEff(eff);
      ...
      for(RooCatType* t = (RooCatType*)catIt->Next(); t!=nullptr; t = (RooCatType*)catIt->Next() ){
        ...
        makeEfficiencyHist1D(myEff, *v1, TString::Format("%s_HIST_%s", v1->GetName(), catName.Data()), catName, effName, "allCats1D", t->getVal());
      }
    }
  }
}
...
void TagProbeFitter::makeEfficiencyHist1D(RooDataSet& eff, RooRealVar& v, const TString& plotName, const TString& plotTitle, const TString& effName, const char *catName, int catIndex){
  /*
  TGraphAsymmErrors *p = new TGraphAsymmErrors();
  const RooArgSet *entry = eff.get();
  const RooRealVar &vi = dynamic_cast<const RooRealVar &>(*entry->find(v.GetName()));
  ...
  */
  ...
  TH1F* h = new TH1F(plotName, plotName, v.getBinning().numBins(), v.getBinning().array());
  const RooArgSet* set = eff.get();
  RooRealVar* e = (RooRealVar*) set->find("efficiency");
  RooRealVar* v_ = (RooRealVar*) set->find(v.GetName());
  //h->SetTitle(TString::Format("%s;%s%s;%s%s;Efficiency of %s", plotTitle.Data(), v.GetTitle(), TString(v.getUnit()).Length()==0?"":TString::Format(" (%s)", v.getUnit()).Data()));
  ...
  for(int i=0; i<eff.numEntries(); i++){
    ...
    if (catName != nullptr && entry->getCatIndex(catName) != catIndex) continue;
    h->SetBinContent(h->FindBin(v_->getVal()), e->getVal());
    h->SetBinError(h->FindBin(v_->getVal()), (e->getErrorHi()-e->getErrorLo())/2.);
  }
  h->Draw();
  canvas.Draw();
  canvas.Write();
