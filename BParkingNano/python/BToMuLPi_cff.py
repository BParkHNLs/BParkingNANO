...
BToMuMuPi = cms.EDProducer(
    'BToMuMuPiBuilder',
    primaryMuons            = cms.InputTag('muonTrgSelector', 'SelectedMuons'),
    ...
    primaryMuonSelection_loose = cms.string('pt > 0 && abs(eta) < 5'),
    ...
    primaryMuonSelection_dsa   = cms.string('pt > 7 && abs(eta) < 1.5'),
...
BToMuMuPiTable = cms.EDProducer(
    ...
    variables=cms.PSet(
        # pre-fit quantities
        CandVars,
        mu0_idx = uint('mu0_idx'),
        mu_idx  = uint('lep_idx'), 
        pi_idx  = uint('pi_idx'), 
        ## trigger muon
        mu0_pt       = ufloat('mu0_pt'   ), 
        mu0_eta      = ufloat('mu0_eta'  ), 
        mu0_phi      = ufloat('mu0_phi'  ), 
        ## pion
        ...
        pi_mu0_vzdiff   = ufloat('pion_mu0_vzdiff'  ),
        # post-fit quantities
        ## vertex information 
        ...
        dr_mu0_hnl    = ufloat('dr_mu0_hnl'     ),
        dr_mu0_mu     = ufloat('dr_mu0_lep'     ),
        dr_mu0_pi     = ufloat('dr_mu0_pi'      ),
        deta_mu_pi    = ufloat('deta_lep_pi'    ),
        deta_mu0_hnl  = ufloat('deta_mu0_hnl'   ),
        deta_mu0_mu   = ufloat('deta_mu0_lep'   ),
        deta_mu0_pi   = ufloat('deta_mu0_pi'    ),
        dphi_mu_pi    = ufloat('dphi_lep_pi'    ),
        dphi_mu0_hnl  = ufloat('dphi_mu0_hnl'   ),
        dphi_mu0_mu   = ufloat('dphi_mu0_lep'   ),
        dphi_mu0_pi   = ufloat('dphi_mu0_pi'    ),
        # Other quantities
        ...
        ## invariant mass
        mu0_mu_mass   = ufloat('dilepton_mass'),
        mu0_pi_mass   = ufloat('mu0_pi_mass'  ),
        mu0_mu_pt     = ufloat('dilepton_pt'  ),
        mu0_pi_pt     = ufloat('mu0_pi_pt'    ),
        ## cos(theta*)
        ...
        mu0_isMatched            = Var("userInt('mu0_isMatched')"           , int, mcOnly=True),
        mu_isMatched             = Var("userInt('lep_isMatched')"           , int, mcOnly=True),
        pi_isMatched             = Var("userInt('pi_isMatched')"            , int, mcOnly=True),
        isBu                     = Var("userInt('BMother_isBu')"            , int, mcOnly=True),
        isBd                     = Var("userInt('BMother_isBd')"            , int, mcOnly=True),
        isBs                     = Var("userInt('BMother_isBs')"            , int, mcOnly=True),
        matching_mu_genIdx       = Var("userInt('matching_lep_genIdx')"     , int, mcOnly=True),
        matching_mu0_genIdx      = Var("userInt('matching_mu0_genIdx')"     , int, mcOnly=True),
        matching_pi_genIdx       = Var("userInt('matching_pi_genIdx')"      , int, mcOnly=True),
        matching_mu_motherPdgId  = Var("userInt('matching_lep_motherPdgId')" , int, mcOnly=True),
        matching_mu0_motherPdgId = Var("userInt('matching_mu0_motherPdgId')", int, mcOnly=True),
        matching_pi_motherPdgId  = Var("userInt('matching_pi_motherPdgId')" , int, mcOnly=True),
        ## reco/gen relative difference
        mupi_mass_reco_gen_reldiff = ufloat('pilep_mass_reco_gen_reldiff'),
